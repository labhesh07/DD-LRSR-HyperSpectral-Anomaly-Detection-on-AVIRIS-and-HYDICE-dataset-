...
def result_show(bg_dic, tg_dic, Z, S, E, rows, cols, bands, bg_dic_label, tg_dic_label):
    """
    Result visualization module displaying the decomposition components.
    
    Methodology Reference: Section 3.1 & 3.9
    - Reconstructs background and anomaly components from decomposition
    - Displays anomaly scores for visualization before thresholding (Equation 19)
    
    :param bg_dic: the background dictionary D_bg
    :param tg_dic: the anomaly dictionary D_an
    ...
    :return: background2d: the 2D background component (D_bg Z)
                target2d : the 2D anomaly component (D_an S)
    
    """
    # Reconstruct background component: D_bg Z from Equation (1)
    background2d = np.dot(bg_dic, Z)
    background3d = hyper.hyperconvert3d(background2d, rows, cols, bands)
    # Reconstruct anomaly component: D_an S from Equation (1)
