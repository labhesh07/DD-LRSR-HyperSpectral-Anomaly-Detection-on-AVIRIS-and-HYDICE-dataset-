...
def dic_constr(data3d, groundtruth, win_size, cluster_num, K, selected_dic_percent, target_dic_num):
    """
    Dictionary Construction Module implementing Sections 3.2-3.6 of the methodology:
    - Section 3.2: Preprocessing (reshaping, normalization, PCA)
    - Section 3.3: Spatial Window Extraction and Clustering
    - Section 3.4: Joint Sparse Representation (JSR)
    - Section 3.5: Background Dictionary Construction
    - Section 3.6: Anomaly Dictionary Construction
    ...
    """
    # A2: Convert hyperspectral cube H×W×B to B×N matrix (each column = one pixel spectrum)
    # Section 3.2.1: Reshaping - Convert H×W×B cube to B×N matrix
    data2d = hyper.hyperconvert2d(data3d)
    rows, cols, bands = data3d.shape
    # Section 3.2.1: Normalization - L2-normalize each spectral vector
    ...
    # A3: PCA dimensionality reduction (for K-means only; full bands kept for SOMP/JSR)
    # Section 3.2.2: Dimensionality Reduction - PCA to retain top d=20 components
    # Equation (2): X_pca = U_d^T X
    ...
    # A4: Spatial window creation for K-means — overlapping w×w patches on PCA-reduced cube
    # Section 3.3: Spatial Window Extraction - Extract overlapping w×w windows
    # Equation (3): W_i ∈ R^(B×w²)
    win_dim = hyper.hyperwincreat(data3d_dim, win_size)
    # A5: K-means clustering — partition window vectors into C material/texture groups
    # Section 3.3: Clustering - K-means over window vectors into C clusters
    # Equation (4): {W_1, W_2, …, W_C}
    cluster_assment = hyper.Kmeans_win(win_dim, cluster_num)
    # A6: Cluster assignment saved — pixel/window label per cluster (used later by result_show)
    sio.savemat("cluster_assment.mat", {'cluster_assment': cluster_assment})
    # Section 3.3: Extract windows from original data (not PCA-reduced) for A7/A8 JSR+SOMP
    ...
    # A8: JSR (Joint Sparse Representation) — per-cluster W_c = D_c·A_c + R_c; A7 SOMP inside loop
    # Section 3.4 & 3.5: Process each cluster for Joint Sparse Representation
    for i in range(cluster_num):
        ...
        if tmp[0].size == 0:
            ...
            # Equation (5): W_c = D_c A_c + R_c
            # Equation (6): arg min_(A_c) ||W_c - D_c A_c||_F
            for j in range(cd_n):
                X = class_data[:, :, j]
                dictionary[:, (j * cd_cols): (j*cd_cols + cd_cols - 1)] = 0
                # A7: SOMP — Simultaneous Orthogonal Matching Pursuit (sparse atom selection per window)
            ...
            # Section 3.5: Background Dictionary Construction
            # Calculate sparsity-frequency score for each atom
            # Equation (7): Score(j) = mean(|a_j|) / freq(j)
            posti_class_global_alpha = np.fabs(class_global_alpha)
            data_frequency = class_global_frequency[:, 0, :]
            frequency = np.sum(data_frequency, axis=1)  # freq(j): frequency of atom j
            sum_frequency = np.sum(frequency)
            norm_frequency = frequency/sum_frequency
            data_mean_alpha = np.mean(posti_class_global_alpha, axis=1)  # mean(|a_j|)
            ...
            # Equation (8): Select Top-p% atoms by score for background dictionary
    ...
    # Section 3.6: Anomaly Dictionary Construction
    # Identify pixels with high JSR residuals as anomaly candidates
    # Equation (9): ||r_i||_2 > τ_r
    ...
    # Equation (10): D_an = {x_i | Residual(x_i) > τ_r}
    # Select subset of K_an atoms for compact anomaly dictionary
